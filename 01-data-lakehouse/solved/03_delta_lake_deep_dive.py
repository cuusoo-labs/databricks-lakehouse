...
# MAGIC %sql
# MAGIC ALTER TABLE
# MAGIC   health_tracker_bronze
# MAGIC REPLACE COLUMNS
# MAGIC   ( 
# MAGIC     device_id LONG COMMENT "range 0 - 4",
# MAGIC     heartrate DOUBLE COMMENT "beats per minute",
...
# MAGIC %sql
# MAGIC DESCRIBE EXTENDED health_tracker_bronze
...
display(dbutils.fs.ls(f"{health_tracker}/bronze"))
...
display(dbutils.fs.ls(f"{health_tracker}/bronze/_delta_log"))
...
display(spark.read.json(f"{health_tracker}/bronze/_delta_log/00000000000000000000.json"))

# COMMAND ----------

# metadata about the table size
display(spark.read.json(f"{health_tracker}/bronze/_delta_log/00000000000000000000.crc"))

# COMMAND ----------

# the second operation performed
display(spark.read.json(f"{health_tracker}/bronze/_delta_log/00000000000000000001.json"))

# COMMAND ----------

# metadata about the table size
display(spark.read.json(f"{health_tracker}/bronze/_delta_log/00000000000000000001.crc"))
...
# MAGIC %sql
# MAGIC 
# MAGIC UPDATE health_tracker_bronze
# MAGIC SET device_id = 5 
# MAGIC WHERE device_id = 0 
...
display(spark.read.format("delta").load(f"{health_tracker}/bronze"))
...
display(spark.read.json(f"{health_tracker}/bronze/_delta_log/00000000000000000002.json"))

# COMMAND ----------

# see the new table stats
display(spark.read.json(f"{health_tracker}/bronze/_delta_log/00000000000000000002.crc"))
...
deltaTable = DeltaTable.forName(spark, "health_tracker_bronze")
...
# MAGIC %sql
# MAGIC -- read the most recent version to compare (version 2)
# MAGIC SELECT 
# MAGIC   COUNT(*), 
# MAGIC   device_id 
# MAGIC FROM 
...
display(spark.read.format("parquet").load(f"{health_tracker}/bronze/_delta_log/00000000000000000010.checkpoint.parquet"))
...
deltaTable = DeltaTable.forName(spark, "health_tracker_bronze")
deltaTable.vacuum() # we attempt to perform a vacuum here, however no files would actually be removed since we haven't exceeded the deletedFileRetentionDuration yet. 
